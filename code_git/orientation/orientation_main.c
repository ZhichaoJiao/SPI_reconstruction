...
int main(int argc, char *argv[])
{
    ...
    int initial_mode = 0; // 初始volume,包含三种模式.1:import_volume;2:random_angle;3:real_angle.
    ...
    char output[400];
    char real_angle_path[400]; // 衍射图准确取向的路径
    int plot_result = 0;       // 是否使用python画图,需要python的画图文件在当前路径下
    ...
    while ((c = getopt_long(argc, argv, "a", longopts, NULL)) != -1)
    {
        ...
        case 14:
            snprintf(real_angle_path, 400, "%s", optarg);
            break;
        ...
        case 17:
            plot_result = strtol(optarg, &rval, 10);
            break;
        }
    }
    ...
    if (0 == world_rank)
    {
        ...
        fprintf(paramater_file, "initial_mode:(1)import_volume;(2)random_angle;(3)real_angle.\n");
        fprintf(paramater_file, "size = %d\n", size);
        fprintf(paramater_file, "pattern_path : %s\n", pattern_path);
        fprintf(paramater_file, "initial_volume_path : %s\n", initial_volume_path);
        fprintf(paramater_file, "real_angle_path : %s\n", real_angle_path);
        ...
        fprintf(paramater_file, "n_loop = %d\n", n_loop);
        fprintf(paramater_file, "initial_mode = %d\n", initial_mode);
    ...
    if (0 == world_rank)
    {
        ...
        else if (3 == initial_mode)
        {
            // 按照准确取向Merge
            hid_t file_real_angle, dataset, dataspace;
            herr_t status;
            file_real_angle = H5Fopen(real_angle_path, H5F_ACC_RDONLY, H5P_DEFAULT);
            dataset = H5Dopen(file_real_angle, "angle", H5P_DEFAULT);
            ...
            for (int index_pattern = 0; index_pattern < N_pattern; index_pattern++)
            {
                ...
                // 输入正确取向
                rot_angle[0] = real_angle[index_pattern][1];
                rot_angle[1] = real_angle[index_pattern][2];
                rot_angle[2] = real_angle[index_pattern][3];
        ...
        hid_t file_initial_volume;
        hid_t dataset, dataspace;
        herr_t status;
        char file_out_volume_path[400] = {0};
        snprintf(file_out_volume_path, 400, "%s/initial_volume.h5", output);
        file_initial_volume = H5Fcreate(file_out_volume_path, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
        hsize_t dim_3[3] = {size, size, size};
        dataspace = H5Screate_simple(3, dim_3, NULL);
        dataset = H5Dcreate(file_initial_volume, "volume_pow", H5T_NATIVE_FLOAT, dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        status = H5Dwrite(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, volume_3d_float);
        H5Dclose(dataset);
        H5Sclose(dataspace);
        status = H5Fclose(file_initial_volume);
    ...
    // for循环1,迭代修改三维volume
    for (int index_loop = 1; index_loop < n_loop + 1; index_loop++)
    {
        // 创建输出文件夹
        char output_dir_i[400] = {0};
        snprintf(output_dir_i, 400, "%s/iteration_%d", output, index_loop);
        if (0 == world_rank)
        {
            printf("\n---------------   Interation %d Begin   ---------------\n", index_loop);
            mkdir(output_dir_i, 0777);
        }
        ...
        for (int index_pattern = 0; index_pattern < N_pattern; index_pattern++)
        {
            ...
            // 输出前三张衍射图的cc系数分布
            if (0 == index_pattern || 1 == index_pattern || 2 == index_pattern)
            {
                MPI_Barrier(MPI_COMM_WORLD);
                ...
                if (0 == world_rank)
                {
                    ...
                    snprintf(file_out_cc_path, 400, "%s/cc_pattern_%d.h5", output_dir_i, index_pattern);
        ...
        if (0 == world_rank)
        {
            // bug3: 判断best_angle_matrix数组中角度排列是否正确,如果出现错误,输出错误代码
            for (int index_error = 0; index_error < N_pattern; index_error++)
            {
                if (index_error * 1.0 != best_angle_matrix_i[index_error][0])
                {
                    printf("\nError: Bug 3\nbest_angle_matrix_i[%d]=%f\n", index_error, best_angle_matrix_i[index_error][0]);
                    return 0;
                }
            }
            ...
            memset(volume_3d, 0, sizeof(double) * size * size * size);
            float(*volume_3d_float)[size][size] = (float(*)[size][size])malloc(sizeof(float) * size * size * size); // 三维衍射强度三维数组
            memset(volume_3d, 0, sizeof(float) * size * size * size);
            memset(volume_1d, 0, sizeof(float) * size * size * size);
            double *weight = (double *)malloc(sizeof(double) * size * size * size); // 每个三维像素点的总权重
            memset(weight, 0, sizeof(double) * size * size * size);
            ...
            for (int index_pattern = 0; index_pattern < N_pattern; index_pattern++)
            {
                ...
                rot_angle[0] = best_angle_matrix_i[index_pattern][1];
                rot_angle[1] = best_angle_matrix_i[index_pattern][2];
                rot_angle[2] = best_angle_matrix_i[index_pattern][3];
            ...
            hid_t dataspace, dataset;
            char file_out_volume_path[400] = {0};
            snprintf(file_out_volume_path, 400, "%s/result_%d.h5", output_dir_i, index_loop);
            file_out_volume = H5Fcreate(file_out_volume_path, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
            // volume
            Volume3dDouble2FLoat(volume_3d, volume_3d_float);
            hsize_t dim_3[3] = {size, size, size};
            dataspace = H5Screate_simple(3, dim_3, NULL);
            dataset = H5Dcreate(file_out_volume, "volume_pow", H5T_NATIVE_FLOAT, dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
            status = H5Dwrite(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, volume_3d_float);
            H5Dclose(dataset);
            H5Sclose(dataspace);
            // best_angle_matrix
            ...
            {
                char command[1000];
                sprintf(command, "/home/jiao/anaconda3/envs/orientation/bin/python ./plot_result_for_c.py --step_beta=%f --i_iteration=%d \
            --n_gamma=%d --n_pattern=%d --result_path=%s --real_angle_path=%s",
                        step, index_loop, n_gamma, N_pattern, output, real_angle_path);
                system(command);
            }
